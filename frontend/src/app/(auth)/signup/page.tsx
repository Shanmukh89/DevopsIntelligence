...
export default function SignUpPage() {
  ...
  const handleEmailSignup = async (e: React.FormEvent) => {
    ...
    // In a real app we'd pass first/last name to user metadata
    const { error } = await supabase.auth.signInWithOtp({
      email,
      options: {
        emailRedirectTo: `${window.location.origin}/dashboard`,
        data: {
          // If you have inputs for these, hook them up to state
    ...
    <div className="bg-[#111318]/90 backdrop-blur-xl border border-white/10 rounded-2xl p-8 shadow-2xl relative overflow-hidden">
      ...
      <div className="relative z-10">
        <div className="text-center mb-8">
          <h2 className="text-xl font-medium text-white mb-2">Create an account</h2>
          <p className="text-sm text-gray-400">Join thousands of developers using Auditr</p>
        </div>
        ...
        <div className="relative mb-6">
          ...
          <div className="relative flex justify-center text-xs uppercase">
            <span className="bg-[#111318] px-2 text-gray-500 font-mono">Or register with email</span>
          </div>
        </div>
        ...
        <form className="space-y-4" onSubmit={handleEmailSignup}>
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                required
                disabled={isLoading !== null}
                className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-sm text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 placeholder:text-gray-600 transition-all font-mono"
              />
            </div>
            <div>
          ...
          <button
            type="submit"
            disabled={isLoading !== null}
            className="w-full h-11 bg-blue-600 hover:bg-blue-500 text-white font-medium text-sm rounded-lg flex items-center justify-center gap-2 transition-colors disabled:opacity-75 shadow-[0_0_15px_rgba(37,99,235,0.3)] mt-2"
          >
        ...
        <div className="mt-6 flex flex-col gap-3">
          <div className="flex items-start gap-2 text-xs text-gray-500">
            ...
            <span>Personal project. All features available.</span>
          </div>
        </div>
      </div>

      <div className="mt-8 text-center text-sm text-gray-400 relative z-10 border-t border-white/10 pt-6">
